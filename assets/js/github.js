/* ============================================================
   github.js — publicar la colección directamente en el repo
   ------------------------------------------------------------
   Opcional. Si configuras un token, el botón "Publicar" escribe
   data/coleccion.json en GitHub mediante la API de contenidos y
   GitHub Pages se regenera solo.

   El token se guarda ÚNICAMENTE en el localStorage de tu navegador;
   nunca se sube al repo. Usa un token de acceso personal de
   grano fino, limitado a este repositorio y con el permiso
...
(function (global) {
  ...
  GH.guardarConfig = function (cfg) { U.guardarLocal(CLAVE, cfg); };
  GH.olvidar = function () { U.borrarLocal(CLAVE); };
  GH.configurado = function () { return !!GH.config().token; };
  ...
  GH.probar = function () {
    var cfg = GH.config();
    return fetch(API + '/repos/' + cfg.owner + '/' + cfg.repo, { headers: cabeceras(cfg) })
      .then(function (r) {
  ...
  function shaActual(cfg) {
    return fetch(urlContenido(cfg) + '?ref=' + encodeURIComponent(cfg.rama), { headers: cabeceras(cfg) })
      .then(function (r) {
  ...
  /** Publica la colección en el repositorio. Devuelve la URL del commit. */
  GH.publicar = function (coleccion, mensaje) {
    var cfg = GH.config();
    if (!cfg.token) return Promise.reject(new Error('No hay token configurado.'));

    var contenido = aBase64(JSON.stringify(coleccion, null, 2) + '\n');

    return shaActual(cfg).then(function (sha) {
      var cuerpo = {
        message: mensaje || 'Actualizar colección de mangas',
        content: contenido,
        branch: cfg.rama
      };
      ...
      return fetch(urlContenido(cfg), {
        method: 'PUT',
        headers: Object.assign({ 'Content-Type': 'application/json' }, cabeceras(cfg)),
